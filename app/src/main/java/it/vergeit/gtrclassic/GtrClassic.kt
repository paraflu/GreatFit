...
import it.vergeit.gtrclassic.widget.*
 ...
 * Amazfit watch faces
 */
class GtrClassic : AbstractWatchFace() {
    private val greatWidget: GreatWidget? = null
    override fun onCreate() {
        instance = WeakReference<GtrClassic?>(this)
        // Load settings
...
//        }
    ...
    override fun slptClockClass(): Class<out AbstractSlptClock?> {
        return GtrClassicSlpt::class.java
    }

    companion object {
        private var instance: WeakReference<GtrClassic?>? = null
        fun getGreatWidget(): GreatWidget? {
            val weakReference: WeakReference<*>? = instance
            return if (weakReference != null) {
                (weakReference.get() as GtrClassic?)!!.greatWidget
            } else null
        }
    }
}