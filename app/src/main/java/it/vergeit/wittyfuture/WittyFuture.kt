 ...
 * Amazfit watch faces
 */
class WittyFuture : AbstractWatchFace() {
    private val greatWidget: GreatWidget? = null
    override fun onCreate() {
        instance = WeakReference<WittyFuture?>(this)
        // Load settings
...
//        }
    ...
    override fun slptClockClass(): Class<out AbstractSlptClock?> {
        return WittyFutureSlpt::class.java
    }

    companion object {
        private var instance: WeakReference<WittyFuture?>? = null
        fun getGreatWidget(): GreatWidget? {
            val weakReference: WeakReference<*>? = instance
            return if (weakReference != null) {
                (weakReference.get() as WittyFuture?)!!.greatWidget
            } else null
        }
    }
}