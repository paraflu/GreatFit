 ...
 * Amazfit watch faces
 */
class OverigBlack : AbstractWatchFace() {
    private val greatWidget: GreatWidget? = null
    override fun onCreate() {
        instance = WeakReference<OverigBlack?>(this)
        // Load settings
        val settings = LoadSettings(applicationContext, "black")
...
//        }
    ...
    override fun slptClockClass(): Class<out AbstractSlptClock?> {
        return OverigBlackSlpt::class.java
    }

    companion object {
        private var instance: WeakReference<OverigBlack?>? = null
        fun getGreatWidget(): GreatWidget? {
            val weakReference: WeakReference<*>? = instance
            return if (weakReference != null) {
                (weakReference.get() as OverigBlack?)!!.greatWidget
            } else null
        }
    }
}