 ...
 * Amazfit watch faces
 */
class Galaxian : AbstractWatchFace() {
    private val greatWidget: GreatWidget? = null
    override fun onCreate() {
        instance = WeakReference<Galaxian?>(this)
        // Load settings
...
//        }
    ...
    override fun slptClockClass(): Class<out AbstractSlptClock?> {
        return GalaxianSlpt::class.java
    }

    companion object {
        private var instance: WeakReference<Galaxian?>? = null
        fun getGreatWidget(): GreatWidget? {
            val weakReference: WeakReference<*>? = instance
            return if (weakReference != null) {
                (weakReference.get() as Galaxian?)!!.greatWidget
            } else null
        }
    }
}