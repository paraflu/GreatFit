    ...
    override fun buildSlptViewComponent(service: Service?, better_resolution: Boolean): List<SlptViewComponent?>? {
        ...
        if (settings.heart_rate > 0) { // Show or Not icon
            if (settings.heart_rateIcon) {
                ...
                heart_rateIcon.setImagePicture(SimpleFile.readFileFromAssets(service, (if (better_resolution) "26wc_" else "slpt_") + "icons/" + settings.is_white_bg + "heart_rate.png"))
        ...
        if (settings.heart_rateProg > 0 && settings.heart_rateProgType == 0) { // Draw background image
            if (settings.heart_rateProgBgBool) {
                ...
                ring_background.setImagePicture(SimpleFile.readFileFromAssets(service, (if (better_resolution) "" else "slpt_") + "circles/ring1_bg.png"))
            ...
            localSlptArcAnglePicView.setImagePicture(SimpleFile.readFileFromAssets(service, (if (better_resolution) "" else "slpt_") + settings.heart_rateProgSlptImage))
